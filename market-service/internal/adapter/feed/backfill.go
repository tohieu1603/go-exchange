...
// Backfill seeds historical OHLCV from Bybit via the candle repository.
type Backfill struct {
	repo domain.CandleRepository
}

func NewBackfill(repo domain.CandleRepository) *Backfill { return &Backfill{repo: repo} }
...
func (b *Backfill) Run() {
	...
	intervals := []struct {
		...
		{"1m", 200}, {"5m", 200}, {"15m", 200}, {"1h", 200}, {"4h", 200}, {"1D", 200}, {"1W", 100},
	}
...
func (b *Backfill) backfillCoin(client *http.Client, coin types.Coin, interval string, limit int) error {
	bybitIv, ok := bybitIntervals[interval]
	if !ok {
		return fmt.Errorf("unsupported interval: %s", interval)
	}
	url := fmt.Sprintf("https://api.bybit.com/v5/market/kline?category=spot&symbol=%sUSDT&interval=%s&limit=%d", coin.Symbol, bybitIv, limit)
	...
	candles := make([]domain.Candle, 0, len(result.Result.List))
	for _, row := range result.Result.List {
		...
		candles = append(candles, domain.Candle{
			Pair: pair, Interval: interval, OpenTime: time.UnixMilli(ts).UTC(),
	...
	if err := b.repo.UpsertBatch(context.Background(), candles); err != nil {
