...
func NewKYCService(kycRepo KYCRepo, userRepo UserRepo, rdb *redis.Client, mail mailer.Mailer) *KYCService {
	if mail == nil {
...
type KYCStatusResponse struct {
	...
	Profile       *domain.KYCProfile   `json:"profile,omitempty"`
	Documents     []domain.KYCDocument `json:"documents"`
}
...
func (s *KYCService) SubmitProfile(userID uint, req SubmitProfileReq) error {
	...
	existing, err := s.kycRepo.FindProfileByUserID(context.Background(), userID)
	...
	if existing != nil && existing.ID != 0 {
		...
		if err := s.kycRepo.UpdateProfile(context.Background(), profile); err != nil {
			return fmt.Errorf("update profile: %w", err)
		}
	} else {
		if err := s.kycRepo.CreateProfile(context.Background(), profile); err != nil {
			return fmt.Errorf("create profile: %w", err)
		}
	}

	if err := s.userRepo.UpdateField(context.Background(), userID, "kyc_step", 2); err != nil {
		return err
	}
...
func (s *KYCService) UploadDocument(userID uint, docType, filePath string) error {
	...
	existing, err := s.kycRepo.FindDocumentByUserAndType(context.Background(), userID, docType)
	if err == nil && existing.ID != 0 {
		if err := s.kycRepo.UpdateDocumentStatus(context.Background(), existing.ID, "PENDING", ""); err != nil {
			return fmt.Errorf("reset doc status: %w", err)
		}
		// Update file path via direct model update
		doc := &domain.KYCDocument{ID: existing.ID, UserID: userID, DocType: docType, FilePath: filePath, Status: "PENDING"}
		_ = doc // handled below
	}

	doc := &domain.KYCDocument{UserID: userID, DocType: docType, FilePath: filePath, Status: "PENDING"}
	if existing != nil && existing.ID != 0 {
		doc.ID = existing.ID
	}
	if err := s.kycRepo.CreateDocument(context.Background(), doc); err != nil {
		return fmt.Errorf("save document: %w", err)
	}

	// Check if all 3 required docs are present
	docs, err := s.kycRepo.FindDocumentsByUserID(context.Background(), userID)
	if err != nil {
...
func (s *KYCService) ApproveKYC(userID uint, adminNote string) error {
	...
	if err := s.kycRepo.UpdateAllDocumentsStatus(context.Background(), userID, "APPROVED", adminNote); err != nil {
...
func (s *KYCService) RejectKYC(userID uint, adminNote string) error {
	...
	if err := s.kycRepo.UpdateAllDocumentsStatus(context.Background(), userID, "REJECTED", adminNote); err != nil {
...
func (s *KYCService) GetKYCStatus(userID uint) (*KYCStatusResponse, error) {
	...
	profile, err := s.kycRepo.FindProfileByUserID(context.Background(), userID)
	if err == nil {
...
func (s *KYCService) GetPendingUsers(page, size int) ([]domain.User, int64, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 || size > 100 {
	...
	return s.kycRepo.FindPendingUsers(context.Background(), page, size)
}
