...
func (s *APIKeyService) Create(userID uint, label, perms, ipWhitelist string, expiresAt *time.Time) (*CreateAPIKeyResult, error) {
	...
	if err := s.repo.Create(context.Background(), k); err != nil {
		return nil, err
	}
	return &CreateAPIKeyResult{Key: k, Secret: secret}, nil
}

func (s *APIKeyService) List(userID uint) ([]domain.APIKey, error) {
	return s.repo.ListByUser(context.Background(), userID)
}

func (s *APIKeyService) Revoke(userID, id uint) error {
	return s.repo.Revoke(context.Background(), id, userID)
}
...
func (s *APIKeyService) Authenticate(keyID, signature, timestamp, method, path string, body []byte, clientIP string) (*domain.APIKey, error) {
	k, err := s.repo.FindByKeyID(context.Background(), keyID)
	if err != nil {
	...
	_ = s.repo.UpdateLastUsed(context.Background(), k.ID, clientIP)
	return k, nil
}

// HasPermission tests whether `perm` is in the comma-separated permissions list.
func HasPermission(k *domain.APIKey, perm string) bool {
...
func normalizePermissions(p string) string {
	...
	allowed := map[string]bool{
		domain.APIPermRead: true, domain.APIPermTrade: true, domain.APIPermWithdraw: true,
	}
