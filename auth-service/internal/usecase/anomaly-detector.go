...
func (a *AnomalyDetector) lastSuccessIP(userID uint, currentIP string) string {
	rows, _, err := a.auditRepo.ListByUser(context.Background(), userID, 1, 10)
	if err != nil {
		return ""
	}
	for _, r := range rows {
		if r.Action != domain.AuditLoginSuccess || r.Outcome != domain.AuditOutcomeSuccess {
			continue
		}
...
func (a *AnomalyDetector) alertEmail(userID uint, email, subject, detail string, when time.Time) {
	user, err := a.users.FindByID(context.Background(), userID)
	if err != nil {
