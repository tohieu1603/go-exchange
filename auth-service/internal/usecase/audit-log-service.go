...
func (s *AuditLogService) RecordWithDevice(userID uint, email, action, outcome, ip, ua, deviceID, detail string) {
	...
	if userID > 0 && deviceID != "" {
		newDevice = !s.repo.HasDeviceForUser(context.Background(), userID, deviceID)
	}
	row := &domain.AuditLog{
...
func (s *AuditLogService) Success(userID uint, email, action, ip, ua, detail string) {
	s.Record(userID, email, action, domain.AuditOutcomeSuccess, ip, ua, detail)
}
...
func (s *AuditLogService) SuccessDevice(userID uint, email, action, ip, ua, deviceID, detail string) {
	s.RecordWithDevice(userID, email, action, domain.AuditOutcomeSuccess, ip, ua, deviceID, detail)
}

func (s *AuditLogService) Failure(userID uint, email, action, ip, ua, detail string) {
	s.Record(userID, email, action, domain.AuditOutcomeFailure, ip, ua, detail)
}
...
func (s *AuditLogService) IsNewDeviceForUser(userID uint, deviceID string) bool {
	...
	return !s.repo.HasDeviceForUser(context.Background(), userID, deviceID)
}

// ListByUser returns the user's own audit history (for `GET /api/auth/audit`).
func (s *AuditLogService) ListByUser(userID uint, page, size int) ([]domain.AuditLog, int64, error) {
	return s.repo.ListByUser(context.Background(), userID, page, size)
}

// ListAll is admin-only — full platform audit query.
func (s *AuditLogService) ListAll(action string, page, size int) ([]domain.AuditLog, int64, error) {
	return s.repo.ListAll(context.Background(), action, page, size)
}
...
func (s *AuditLogService) Prune(retentionDays int) int64 {
	...
	return s.repo.PruneOlderThan(context.Background(), retentionDays)
}
