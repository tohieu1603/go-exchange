...
func (s *AuthService) Register(req RegisterReq) (*domain.User, string, string, error) {
	existing, err := s.userRepo.FindByEmail(context.Background(), req.Email)
	...
	s.audit.SuccessDevice(user.ID, user.Email, domain.AuditRegister, req.ClientIP, req.UserAgent, regDeviceID, "")
...
func (s *AuthService) Login(req LoginReq) (*LoginResult, error) {
	...
	if ttl := s.throttle.CheckLocked(ctx, req.Email, req.ClientIP); ttl > 0 {
		s.audit.Failure(0, req.Email, domain.AuditLoginFailure, req.ClientIP, req.UserAgent,
	...
	user, err := s.userRepo.FindByEmail(context.Background(), req.Email)
	if err != nil || user == nil || user.ID == 0 {
		s.throttle.RecordFailure(ctx, req.Email, req.ClientIP)
		s.audit.Failure(0, req.Email, domain.AuditLoginFailure, req.ClientIP, req.UserAgent, "user not found")
		return nil, errors.New("invalid credentials")
	}
	// SYSTEM accounts cannot log in. Reject before bcrypt to leak no signal.
	if user.Role == types.RoleSystem {
		s.audit.Failure(user.ID, req.Email, domain.AuditLoginFailure, req.ClientIP, req.UserAgent, "system account")
		return nil, errors.New("invalid credentials")
	}
	if !utils.CheckPassword(req.Password, user.PasswordHash) {
		_, remaining := s.throttle.RecordFailure(ctx, req.Email, req.ClientIP)
		s.audit.Failure(user.ID, req.Email, domain.AuditLoginFailure, req.ClientIP, req.UserAgent,
	...
	if utils.IsLegacyHash(user.PasswordHash) {
		if newHash, err := utils.HashPassword(req.Password); err == nil {
			_ = s.userRepo.UpdateField(context.Background(), user.ID, "password_hash", newHash)
		}
	}
	...
	if req.ClientIP != "" {
		...
		// email/fullName/etc.
		_ = s.userRepo.UpdateField(context.Background(), user.ID, "last_login_ip", req.ClientIP)
	}
	...
	s.audit.SuccessDevice(user.ID, user.Email, domain.AuditLoginSuccess,
...
func (s *AuthService) LoginWithGoogle(idToken, ip, ua string) (*LoginResult, error) {
	...
	user, err := s.userRepo.FindByGoogleSub(context.Background(), claims.Sub)
	if err != nil || user == nil || user.ID == 0 {
	...
	if isNew {
		...
		s.audit.SuccessDevice(user.ID, user.Email, domain.AuditRegister, ip, ua, regDeviceID, "google")
	} else {
		...
		if len(updates) > 0 {
			_ = s.userRepo.UpdateFields(context.Background(), user.ID, updates)
		}
	}
	...
	if ip != "" {
		_ = s.userRepo.UpdateField(context.Background(), user.ID, "last_login_ip", ip)
	}
	s.rdb.Set(ctx, fmt.Sprintf("kyc:%d", user.ID), user.KYCStep, 0)
	s.rdb.Set(ctx, fmt.Sprintf("user_locked:%d", user.ID), boolStr(user.IsLocked), 0)

	deviceID := utils.DeviceFingerprint(ip, ua, "")
	s.audit.SuccessDevice(user.ID, user.Email, domain.AuditLoginSuccess, ip, ua, deviceID, "google")
	metrics.LoginTotal.WithLabelValues("success").Inc()
...
func (s *AuthService) Login2FA(tempToken, code, ua, ip string) (string, string, *domain.User, error) {
	claims, err := utils.ValidateTempToken(tempToken, s.secret)
	if err != nil {
		return "", "", nil, errors.New("invalid or expired temp token")
	}
	u, err := s.userRepo.FindByID(context.Background(), claims.UserID)
	if err != nil {
...
func (s *AuthService) RefreshToken(rawRefresh, ua, ip string) (string, string, error) {
	...
	if user.IsLocked {
		_ = s.rtSvc.RevokeAllForUser(userID, domain.RevokeReasonAdmin)
...
func (s *AuthService) Logout(rawRefresh string, userID uint, email, ip, ua string) error {
	...
	err := s.rtSvc.Revoke(rawRefresh, domain.RevokeReasonLogout)
	if userID != 0 {
		s.audit.Success(userID, email, domain.AuditLogout, ip, ua, "")
	}
	return err
}
...
func (s *AuthService) LogoutAll(userID uint) error {
	return s.rtSvc.RevokeAllForUser(userID, domain.RevokeReasonAdmin)
}
...
func (s *AuthService) UpdateProfile(userID uint, fullName string) (*domain.User, error) {
	user, err := s.userRepo.FindByID(context.Background(), userID)
	if err != nil {
		return nil, errors.New("user not found")
	}
...
func (s *AuthService) SetInitialPassword(userID uint, newPass string) error {
	...
	if err := s.userRepo.UpdateField(context.Background(), user.ID, "password_hash", hash); err != nil {
		return err
	}
	s.audit.Success(user.ID, user.Email, domain.AuditPasswordChange, "", "", "initial password set")
	return nil
}
...
func (s *AuthService) ChangePassword(userID uint, oldPass, newPass string) error {
	...
	_ = s.rtSvc.RevokeAllForUser(userID, domain.RevokeReasonPasswordChange)
	s.audit.Success(user.ID, user.Email, domain.AuditPasswordChange, "", "", "")
	return nil
}
...
func (s *AuthService) Enable2FA(userID uint) (secret, qrURL string, err error) {
	...
	if err := s.userRepo.UpdateField(context.Background(), userID, "two_fa_secret", secret); err != nil {
		return "", "", err
	}
...
func (s *AuthService) Verify2FA(userID uint, code string) error {
	...
	return s.userRepo.UpdateField(context.Background(), userID, "is2_fa", true)
}
...
func (s *AuthService) Disable2FA(userID uint, code string) error {
	...
	return s.userRepo.UpdateFields(context.Background(), userID, map[string]interface{}{
...
func (s *AuthService) CompleteStepUp(token, code, ua, ip string) (*LoginResult, error) {
	...
	user, err := s.userRepo.FindByID(context.Background(), res.UserID)
	if err != nil {
		return nil, errors.New("user not found")
	}
	access, refresh, err := s.issueTokenPair(user, ua, ip)
	...
	deviceID := utils.DeviceFingerprint(ip, ua, "")
	s.audit.SuccessDevice(user.ID, user.Email, domain.AuditLoginSuccess, ip, ua, deviceID,
		"step-up confirmed")
	return &LoginResult{AccessToken: access, RefreshToken: refresh, User: user}, nil
}

// issueTokenPair generates a fresh access JWT + a brand-new refresh family.
func (s *AuthService) issueTokenPair(u *domain.User, ua, ip string) (string, string, error) {
...
func (s *AuthService) SeedAdmin() {
	...
	if s.userRepo.Count(context.Background()) > 0 {
		return
	}
	...
	admin := &domain.User{Email: "admin@exchange.com", PasswordHash: hash, FullName: "Admin", Role: "ADMIN", KYCStatus: "VERIFIED", KYCStep: 4, EmailVerified: true}
	if err := s.userRepo.Create(context.Background(), admin); err == nil {
		s.bus.Publish(context.Background(), eventbus.TopicUserRegistered, eventbus.UserRegisteredEvent{
	...
	user := &domain.User{Email: "user@exchange.com", PasswordHash: hash2, FullName: "Demo User", Role: "USER"}
	if err := s.userRepo.Create(context.Background(), user); err == nil {
		s.bus.Publish(context.Background(), eventbus.TopicUserRegistered, eventbus.UserRegisteredEvent{
			UserID: user.ID, Email: user.Email, FullName: user.FullName,
			Role:        "USER",
...
func (s *AuthService) EnsureFeeWallet() (uint, error) {
	if u, err := s.userRepo.FindByEmail(context.Background(), types.SystemEmailFeeWallet); err == nil && u != nil && u.ID != 0 {
		return u.ID, nil
	}
	user := &domain.User{
...
func (s *AuthService) CleanupExpiredTokens() {
	...
	if n, err := s.rtSvc.CleanupExpired(context.Background(), cutoff); err == nil && n > 0 {
		log.Printf("[auth] pruned %d expired refresh tokens", n)
	}
}
